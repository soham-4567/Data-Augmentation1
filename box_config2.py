...
img = cv2.imread("3cdbd5d1-AqilaugmentedBlur1.jpg")
angle = 45

h, w = img.shape[:2] #get image width and height
cx, cy = (int(w / 2), int(h / 2)) # get image center
rotation_mat = cv2.getRotationMatrix2D((cx, cy), angle, 1.0)
img_rotated = cv2.warpAffine(src = img, M=rotation_mat, dsize = (w, h))